...
function Albums() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (newAlbum.id === null) {
                await axios.post("http://localhost:8080/album", albumData);
            } else {
                await axios.put(`http://localhost:8080/album/${newAlbum.id}`, albumData);
            }

            const refreshed = await axios.get("http://localhost:8080/albums");
    ...
    const filterAlbums = (album) => {
        ...
        switch (filterOption) {
            ...
            case "Number of Songs":
                return album.numberOfSongs.toString().toLowerCase().includes(input.toLowerCase());
            case "List of Songs":
    ...
    const handleDelete = async (albumId) => {
        ...
        if (confirmDelete) {
            try {
                await axios.delete(`http://localhost:8080/album/${albumId}`);
                const refreshed = await axios.get("http://localhost:8080/albums");
                setAlbums(refreshed.data);
            } catch (error) {
                    ...
                    {albums.filter(filterAlbums).map((album) => (
                        <li key={album.id} className="item">
                            <div className="details">
                                ...
                                <div className="info">
                                    <p><strong>Artist: {album.artist?.name}</strong></p>
                                    <p><strong>Release Year: {album.releaseYear}</strong></p>
                                    <p><strong>Genre: {album.genre}</strong></p>
                                    <p><strong>Number of Songs: {album.numberOfSongs}</strong></p>
                                </div>
